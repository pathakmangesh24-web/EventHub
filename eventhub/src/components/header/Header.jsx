...
const Header = () => {
  ...
  const vendorItems = [
    { name: "Craft Vendor", href: "#craft-vendor" },
    { name: "Food Vendor", href: "#food-vendor" },
      ...
      <div className="max-w-[1350px] mx-auto h-[70px] px-5 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-9 text-[15px] ml-85">
          ...
          <div className="relative">
            ...
            {vendorOpen && (
              <div className="absolute left-0 top-full mt-3 w-52 rounded-lg border border-gray-200 bg-white py-2 shadow-lg z-20">
                {vendorItems.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    onClick={() => setVendorOpen(false)}
                    className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-blue-600 transition"
                  >
                    {item.name}
                  </a>
                ))}
